...
async function load_csv()
{
  ...
  const uri = resIndex.data.substring( lastLinkIndex, config.SAITAMA_CSV.SEARCH_KEY.length + 100 ).match( /<a href="([^"]+)"/ )[ 1 ];
  if ( !uri )
...
export default class PoiSaitama
{
  static async load()
  {
    ...
    for ( let rownum=1; rownum < rows.length; rownum++ )
    {
      ...
      let city = sanitize_poi_name( (row[ 4 ] === prefname) ? '' : row[ 4 ] );
      if ( !map_poi.get( city ) )
      {
        Log.info( `埼玉県${city} not found` );
        continue;
      }
