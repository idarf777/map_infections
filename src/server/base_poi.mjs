...
import {zenkokuData} from "./server.mjs";
...
export default class BasePoi
{
  ...
  static async process_csv( arg )
  {
    ...
    try {
      ...
      // 厚労省の全国データにフォールバックする
      const db = map_poi.get( '' );
      const data = zenkokuData[ 0 ]?.get( db.name_en );
      if ( data )
      {
        const timestamps = Array.from( data.keys() ).sort();
        let subtotal = 0;
        const spots = [ {
          city_code: db.city_cd * 1000,
          getpos: db.geopos(),
          name: "総数",
          data: timestamps.map( v => {
            const infectors = data.get( v );
            subtotal += infectors;
            return { date: datetostring( new Date( v ) ), infectors, subtotal };
          } )
        } ];
        ex.pois = { pref_code: db.city_cd, name: pref_name, begin_at: datetostring( new Date( timestamps[ 0 ] ) ), finish_at: datetostring( new Date( timestamps[ timestamps.length - 1 ] ) ), spots };
      }
      throw ex;
    }
  }
