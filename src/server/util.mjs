...
export async function load_geojson()
{
  const whole = (await axios_instance().get( `${process.env.PUBLIC_URL}/japan.geojson` )).data;
  // 都道府県ごとに分割する
  return whole.features.map( feature => {
    feature.properties.name = feature.properties.name.replace( /[Ōō]/g, 'o' ).toLowerCase();
    return {
      id: `layer-pref-${feature.properties.name}`,
      prefcd: PREFECTURE_NAMES[ feature.properties.name ],
  ...
  } ).sort( (a, b) => a.prefcd - b.prefcd );  // 都道府県コードの昇順
}
